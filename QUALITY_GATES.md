...
上述 `check_workstream_scope.py` 只识别原三条平台 stream，不把本地内容生产伪装成第四个 `--stream` 参数。本地自动混剪的可执行提交前检查是下节列出的定向测试；若改到侧栏、启动参数、路径或其他共享核心，还必须由集成线执行“每次共享核心合并”的完整回归、离屏客户端与差异检查。
