...
- 素材管理：按主体分类管理视频与图片素材。
- 自动混剪：从素材管理或本机选择多条视频，设置成片数量、总时长和单镜头时长后，在本机生成 9:16 硬切成片；严格模式不重复使用同一素材时间窗，素材不足时明确提示可生成上限，也可由用户主动开启受控复用。默认静音以避免把口播从句中切断；“保留环境原声”只允许在用户明确确认素材没有连续讲话后使用，并为每个切片加入短淡入淡出。标题和正文支持用户编写的 `[写法一|写法二]` 变体，不调用模型、不擅自改写原意。成片先供人工查看，不会自动进入平台发布。
