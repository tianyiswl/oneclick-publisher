...
- 当前本地集成线已接入国内定位与抖音评论洞察；评论洞察共享数据库接线提交为 `326a991`。
...
| --- | --- | --- | --- | --- |
| 数据监测 | 已注册抖音、小红书、快手、B站、公众号五个平台的只读采集器；视频号未注册。抖音评论洞察已完成手动单作品只读同步、匿名评论保存、证据型分类候选和共享数据库接线。 | 小红书最近两次同步记录为成功；公众号、B站、快手各有连续两次真实只读同步和数据库/界面读回记录。抖音评论洞察本轮只有本地与集成测试，没有真实平台同步。 | 生产评论合同和清单尚不存在，暂时不能执行真实评论同步；视频号仍为 `collector_not_available`。 | `feature/data-monitoring-v3` |
...
| 集成与发布 | 已本地合并国内定位和数据监测评论洞察；评论表已接入共享数据库启动入口。 | 评论洞察接入后直接受影响测试 `215/215`、完整测试 `1771/1771`，离屏客户端返回 `NATIVE_DESKTOP_UI_OK`。 | 海外登录发布线尚未完成集成；三线完成后再统一递增版本，并做 Mac 真实启动、Windows 候选包和版本一致性检查。 | `integration/oneclick-0.5.0` |
...
继续完成海外登录发布线的质量门并串行合入集成线；三线全部完成后统一递增版本、全量回归和打包。抖音评论洞察在生产合同出现前保持“本地与集成已验证、真实平台未验证”。
