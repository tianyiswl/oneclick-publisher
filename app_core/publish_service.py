...
class PublishServiceError(ValueError):
    """Stable public boundary error raised before a publish worker starts."""
    ...
    for raw in payloads:
        ...
        if platform_type == 6:
            if (
                ...
                and payload.get("overseasVideoPublishConfirmed") is not True
            ):
                raise ValueError("TikTok 正式发布必须先完成当次正式发布确认")
            tiktok_mode = {
                "preflight": "preflight",
                "platform_form_check": "platform_form_check",
                "publish": "formal",
            }.get(runtime_mode)
            if tiktok_mode is None:
                raise ValueError("TikTok 只支持本地预检、平台表单检查或受控正式发布")
            overseas_tiktok_publish.validate_tiktok_payload(
                payload,
                mode=tiktok_mode,
            )
        ...
        if runtime_mode == "preflight":
            ...
            if platform_type == 9:
                overseas_browser_publish._validate_facebook_page_payload(
                    payload,
                    mode="preflight",
                )
        ...
        elif runtime_mode == "platform_form_check":
            if platform_type != 6:
                raise ValueError("TikTok 平台表单检查不能混入其他平台")
            if payload.get("debugDryRun") is not False:
                raise ValueError("TikTok 平台表单检查会写入平台表单")
            payload["backgroundMode"] = False
    ...
    try:
        ...
        try:
            ...
            task_service.mark_platform_result(
                ...
                6,
                ok=bool(result.get("ok")),
                message=str(result.get("message") or "TikTok 表单检查结束"),
                content_type=str(payload.get("contentType") or ""),
                event_type="platform_form_check",
                error_code=str(result.get("errorCode") or ""),
                receipt=(
            ...
            task_service.mark_platform_result(
                ...
                6,
                ok=False,
                message=_failure_message("TikTok 表单检查异常", exc, platform_type=6),
                content_type=str(payload.get("contentType") or ""),
                event_type="platform_form_check",
                error_code=_failure_error_code(exc, platform_type=6),
                receipt=(
                    dict(getattr(exc, "receipt", {}))
                    if isinstance(getattr(exc, "receipt", None), dict)
                    else None
...
def start_desktop_publish(payloads: list[dict[str, Any]]) -> dict:
    ...
    if any(
        ...
        and int(item.get("type") or 0) == 9
        and str(item.get("runtimeMode") or "preflight") == "publish"
        for item in payloads
    ):
        raise PublishServiceError(
            "facebook_publish_authorization_invalid",
            "Facebook Page 正式发布缺少数据库受控任务 claim。",
        )
    ...
    worker = threading.Thread(
        target=(
            _run_publish
            if is_publish
            else _run_platform_form_check
            if is_platform_form_check
            else _run_wechat_draft
            if is_wechat_draft
            else _run_draft
            if is_draft
            else _run_preflight
...
def _facebook_failure_receipt(
    payload: dict[str, Any],
    exc: BaseException | None = None,
) -> dict[str, object]:
    receipt = {
        "pageId": str(payload.get("facebookExpectedPageReference") or ""),
    }
    error_receipt = getattr(exc, "receipt", None)
    if isinstance(error_receipt, dict):
        receipt.update(error_receipt)
        receipt["pageId"] = str(
...
def _mark_facebook_worker_start_failed(
    task_id: int,
    payload: dict[str, Any],
    exc: BaseException,
    *,
...
def _run_facebook_page_publish(
    task: dict[str, Any],
    payloads: list[dict[str, Any]],
) -> None:
    ...
    try:
        ...
        if claim_state != "succeeded" or not bool(result.get("ok")):
            raise FacebookPagePublishError(
                "facebook_publish_outcome_unknown",
                "Facebook Page worker 未取得 claim 与内容列表一致的成功终态。",
                receipt=(
                    result.get("receipt")
                    if isinstance(result.get("receipt"), dict)
                    else _facebook_failure_receipt(payload)
                ),
...
def start_controlled_facebook_publish(
    ...
    *,
    ...
    try:
        ...
        raise PublishServiceError(
            "facebook_publish_authorization_invalid",
            "Facebook Page 受控任务快照不可读取。",
        ) from exc
    if (
        str(task.get("mode") or "") != "oneclick_publish"
