...
class DouyinCommercePayloadTests(unittest.TestCase):
    ...
    def test_saved_location_is_clicked_in_the_same_open_panel(self) -> None:
        ...
        self.assertNotIn("expected_location", search_kwargs[0])
        self.assertEqual(search_kwargs[0]["commission_filter"], "all")
        self.assertGreater(search_kwargs[0]["timeout_ms"], 0)
...
class DouyinCommerceLocationDomTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_publish_load_more_deadline_stops_before_candidate_poll(self) -> None:
        ...
        self.assertLessEqual(timeouts[0], 1_000)
        self.assertLessEqual(timeouts[1], 400)
        self.assertIs(getattr(raised.exception, "click_performed", False), True)
    ...
    async def test_publish_restarts_deadline_for_each_search_load_and_readback(
        ...
        # 搜索前清理也拥有独立期限，因此搜索、分页和回读分别使用后续时刻。
        clock = iter((5.0, 10.0, 15.0, 20.0, 25.0))
        ...
        with patch.object(
            douyin_commerce_service,
            "monotonic",
            side_effect=lambda: next(clock, 25.0),
        ), patch.object(
            douyin_commerce_service,
            "search_commerce_location_store_candidates",
            new=search,
    ...
    async def test_publish_real_search_waiter_returns_first_page_before_loading_target(
        self,
    ) -> None:
        """正式发布搜索等待器必须先交出稳定首屏，目标由外层分页找到。"""
        ...
        self.assertIsNotNone(preset)
        value = {"text": ""}
        input_control = AsyncMock()

        async def fill_input(text, **_kwargs):
            value["text"] = text

        async def read_input(_script):
            return value["text"]

        input_control.fill.side_effect = fill_input
        input_control.evaluate.side_effect = read_input
        page = AsyncMock()
        snapshot_reads = {"count": 0}

        async def read_snapshot(_page):
            snapshot_reads["count"] += 1
            if snapshot_reads["count"] == 1:
                return None, [], "baseline"
            return object(), [dict(first)], "stable-first-page"

        loaded = {
            "platformResultCount": 2,
            "candidates": [dict(first), dict(preset)],
            "newCandidateCount": 1,
            "hasMore": False,
            "stopReason": "loaded",
        }
        ...
        self.assertGreaterEqual(snapshot_reads["count"], 3)
    ...
    async def test_publish_progressive_pages_do_not_share_one_thirty_second_deadline(
        self,
    ) -> None:
        """每次分页均有进展时，累计超过三十秒仍要继续找到第七页目标。"""
        ...
        for index in range(1, 8):
            candidate = (
                preset
                if index == 7
                else douyin_commerce_service.normalize_commerce_location_candidate(
                    {
                        "name": f"第{index}页非目标",
        ...
        with patch.object(
            ...
            result = await douyin_commerce_service.apply_saved_commerce_location_to_page(
                object(), preset, "domestic", ["第七页目标"], "commission"
            )

        self.assertEqual(load_more.await_count, 7)
        self.assertEqual(result["location"]["poiId"], preset["poiId"])

    async def test_publish_reports_action_timeout_with_safe_diagnostic(self) -> None:
        """单次加载超时时只返回受控诊断，不泄露页面或异常原文。"""
        ...
        with patch.object(
            ...
            with self.assertRaises(douyin_commerce_service.DouyinCommerceError) as caught:
                await douyin_commerce_service.apply_saved_commerce_location_to_page(
                    object(), preset, "domestic", ["超时目标"], "commission"
                )

        self.assertEqual(str(caught.exception), "publish_location_action_timeout")
        self.assertEqual(caught.exception.diagnostic["stage"], "load_more")
        self.assertEqual(caught.exception.diagnostic["scope"], "domestic")
        self.assertEqual(
    ...
    async def test_publish_reports_click_limit_with_safe_diagnostic(self) -> None:
        """第十次加载成功后，下一次点击前必须返回点击上限诊断。"""
    ...
    async def test_publish_does_not_count_missing_load_more_control_as_a_click(
        self,
    ) -> None:
        """无分页控件的穷尽回包不能消耗跨关键词点击预算。"""
    ...
    async def test_publish_counts_a_successful_click_before_load_readback_timeout(
        self,
    ) -> None:
        """点击已成功后候选回读超时，超时诊断仍须计入本次点击。"""
        ...
        load_timeout = douyin_commerce_service.DouyinCommerceError(
            "publish_location_load_more_limit"
        )
        load_timeout.click_performed = True
        with patch.object(
            douyin_commerce_service,
            "search_commerce_location_store_candidates",
            new_callable=AsyncMock,
        ...
        self.assertEqual(str(caught.exception), "publish_location_action_timeout")
        self.assertEqual(caught.exception.diagnostic["loadMoreClicks"], 1)

    async def test_publish_reports_candidate_limit_with_safe_diagnostic(self) -> None:
        """一百个唯一候选已到达时，下一次点击前必须返回候选上限诊断。"""
        ...
        with patch.object(
            ...
            with self.assertRaises(douyin_commerce_service.DouyinCommerceError) as caught:
                await douyin_commerce_service.apply_saved_commerce_location_to_page(
                    object(), preset, "domestic", ["候选上限目标"], "commission"
                )
    ...
    async def test_publish_candidate_limit_on_broad_keyword_continues_exact_keyword(
        self,
    ) -> None:
        """宽词扫描满一百条后必须切换精确词，不能终止整条视频。"""
        ...
        with patch.object(
            ...
            "search_commerce_location_store_candidates",
            new_callable=AsyncMock,
            side_effect=[broad_candidates, [preset]],
        ...
        self.assertEqual(result["matchedKeyword"], "夜南香北京烤鸭(南昌县店)")
        self.assertEqual(result["location"]["name"], preset["name"])
        self.assertEqual(search.await_count, 2)
        load_more.assert_not_awaited()

    async def test_publish_load_timeout_on_broad_keyword_continues_exact_keyword(
        self,
    ) -> None:
        """宽词分页超时后必须关闭当前面板并尝试精确词。"""

        broad_candidate = douyin_commerce_service.normalize_commerce_location_candidate(
            {
                "name": "夜南香非目标",
                "address": "广西北海市候选路1号",
                "commerceInfo": "1件商品 · 1件返佣",
        ...
        preset = douyin_commerce_service.normalize_commerce_location_candidate(
            {
                "name": "夜南香北京烤鸭(蕲春店)",
                "address": "湖北省黄冈市蕲春县齐昌大道694号",
                "commerceInfo": "1件商品 · 1件返佣",
            }
        )
        self.assertIsNotNone(broad_candidate)
        self.assertIsNotNone(preset)
        load_timeout = douyin_commerce_service.DouyinCommerceError(
            "publish_location_load_more_limit"
        )
        ...
        with patch.object(
            ...
            "search_commerce_location_store_candidates",
            new_callable=AsyncMock,
            side_effect=[[broad_candidate], [preset]],
        ) as search, patch.object(
            douyin_commerce_service,
            "load_more_commerce_location_candidates",
            new_callable=AsyncMock,
        ...
        self.assertEqual(result["matchedKeyword"], "夜南香北京烤鸭(蕲春店)")
        self.assertEqual(result["location"]["address"], preset["address"])
        self.assertEqual(search.await_count, 2)
    ...
    async def test_publish_shares_ten_click_budget_across_three_keywords(self) -> None:
        """三个兜底关键词合计只能点击十次，不能每词重置预算。"""
        ...
        active_keyword = {"value": ""}
        rounds: dict[str, int] = {}

        def candidate(keyword: str, index: int) -> dict[str, object]:
            return douyin_commerce_service.normalize_commerce_location_candidate(
                {
                    "name": f"{keyword}候选{index}",
                    "address": f"广西北海市{keyword}路{index}号",
                    "commerceInfo": "1件商品 · 1件返佣",
                }
            )
        ...
        async def search(_page, keyword, **_kwargs):
            ...
            rounds[keyword] = 0
            return [candidate(keyword, 0)]

        async def load_more(_page, *, previous_candidates, **_kwargs):
            keyword = active_keyword["value"]
            rounds[keyword] += 1
            return {
                "platformResultCount": len(previous_candidates) + 1,
                "candidates": [
                    *previous_candidates,
                    candidate(keyword, rounds[keyword]),
                ],
                "newCandidateCount": 1,
                "hasMore": rounds[keyword] < 4,
                "stopReason": "loaded",
            }
        ...
        with patch.object(
            ...
            new_callable=AsyncMock,
            side_effect=load_more,
        ) as load_more_mock, patch.object(
            douyin_commerce_service,
            "close_commerce_store_selector",
            new_callable=AsyncMock,
        ):
        ...
        self.assertEqual(load_more_mock.await_count, 10)

    async def test_publish_scopes_hundred_identity_limit_to_each_keyword(
        self,
    ) -> None:
        """不同关键词可各自扫描候选，不能让宽词占满后续精确词额度。"""
        ...
        def candidates(keyword: str, start: int) -> list[dict[str, object]]:
            return [
                douyin_commerce_service.normalize_commerce_location_candidate(
                    {
                        "name": f"{keyword}候选{index}",
        ...
        async def search(_page, keyword, **_kwargs):
            active_keyword["value"] = keyword
            return candidates(keyword, 0)

        async def load_more(_page, *, previous_candidates, **_kwargs):
            keyword = active_keyword["value"]
            return {
                "platformResultCount": 40,
                "candidates": [*previous_candidates, *candidates(keyword, 20)],
        ...
        self.assertEqual(caught.exception.diagnostic["keyword"], "关键词丙")
        self.assertEqual(caught.exception.diagnostic["candidateCount"], 40)
        self.assertEqual(load_more_mock.await_count, 3)

    async def test_publish_restarts_action_deadline_across_keywords(self) -> None:
        """前一关键词已完成后，下一次搜索必须拥有新的单次动作期限。"""
        ...
        with patch.object(
            douyin_commerce_service,
            "monotonic",
            side_effect=lambda: next(clock, 31.0),
        ), patch.object(
            douyin_commerce_service,
            "search_commerce_location_store_candidates",
            new_callable=AsyncMock,
    ...
    async def test_publish_accepts_hundredth_target_and_ignores_later_same_page_rows(
        self,
    ) -> None:
        """同批第百条目标可命中，第百零一条不得先触发误杀。"""
        ...
        preset = douyin_commerce_service.normalize_commerce_location_candidate(
            {
                "name": "第一百条目标",
                "address": "广西北海市目标路100号",
                "commerceInfo": "1件商品 · 1件返佣",
            }
        )
        over_limit = douyin_commerce_service.normalize_commerce_location_candidate(
            {
                "name": "第一百零一条非目标",
                "address": "广西北海市候选路101号",
                "commerceInfo": "1件商品 · 1件返佣",
            }
        )
        self.assertIsNotNone(preset)
        loaded = {
            ...
            "candidates": [*first_ninety_nine, preset, over_limit],
            "newCandidateCount": 2,
            "hasMore": True,
            "stopReason": "loaded",
        }
        with patch.object(
            ...
            "search_commerce_location_store_candidates",
            new_callable=AsyncMock,
            return_value=first_ninety_nine,
        ), patch.object(
            douyin_commerce_service,
            "load_more_commerce_location_candidates",
            new_callable=AsyncMock,
        ...
        self.assertEqual(result["location"]["poiId"], preset["poiId"])
        load_more.assert_awaited_once()
        apply_open.assert_awaited_once()
    ...
    async def test_publish_all_pages_failure_has_complete_scoped_diagnostic(
        self,
    ) -> None:
        """平台穷尽必须由 service 产生含 scope 的完整九字段诊断。"""
        ...
        with patch.object(
            ...
            with self.assertRaises(douyin_commerce_service.DouyinCommerceError) as caught:
                await douyin_commerce_service.apply_saved_commerce_location_to_page(
                    object(), preset, "local", ["夜南香"], "commission"
                )
    ...
    async def test_publish_process_control_exception_survives_cleanup_failure(
        self,
    ) -> None:
        """分页中断后仍尝试严格清理，但普通清理错误不得覆盖进程控制异常。"""
        ...
        for exception_type in (
            ...
            with self.subTest(exception_type=exception_type.__name__), patch.object(
                douyin_commerce_service,
                "search_commerce_location_store_candidates",
                new_callable=AsyncMock,
                ...
                "close_commerce_store_selector",
                new_callable=AsyncMock,
                side_effect=[None, RuntimeError("cleanup-sensitive-detail")],
            ) as close_selector:
                with self.assertRaisesRegex(exception_type, "process-control"):
                    await douyin_commerce_service.apply_saved_commerce_location_to_page(
                        object(), preset, "domestic", ["夜南香"], "commission"
                    )
    ...
    async def test_publish_gives_preflight_and_final_cleanup_independent_deadlines(
        self,
    ) -> None:
        """搜索前与退出清理各自获得新的三十秒绝对期限。"""
        ...
        with patch.object(
            douyin_commerce_service,
            "_close_commerce_store_selector_strict",
            new_callable=AsyncMock,
        ...
        self.assertEqual(close_selector.await_count, 2)
        deadlines = [
            call.kwargs.get("deadline") for call in close_selector.await_args_list
        ]
        self.assertTrue(all(type(value) is float for value in deadlines))
        self.assertGreater(deadlines[1], deadlines[0])

    async def test_publish_stops_a_slow_preflight_cleanup_at_its_deadline(
        self,
    ) -> None:
        """搜索前清理耗尽三十秒后必须停止，不能继续启动搜索。"""

        clock = {"now": 0.0}

        async def slow_search_input(_page):
            clock["now"] += 31.0
            return None
    ...
    async def test_publish_keeps_original_failure_when_final_cleanup_also_fails(
        self,
    ) -> None:
        """业务失败优先于 finally 的普通清理失败。"""
    ...
    async def test_publish_continues_after_commission_mismatch_and_finds_later_page(
        self,
    ) -> None:
        """首屏同 POI 异佣只是证据，第二批出现完整四字段目标时仍须成功。"""
    ...
    async def test_load_more_does_not_escape_panel_to_shared_editor_button(
        self,
    ) -> None:
        """地点面板没有按钮时，不得上爬到共享编辑器点击无关控件。"""
    ...
    async def test_load_more_clicks_plain_text_row_with_delegated_handler(
        self,
    ) -> None:
        """平台用普通 div/span 和事件代理时也必须真实点击加载。"""
        ...
        <section role="dialog">
          <div id="current-location-content">
            <nav><span>本地</span><span>国内</span></nav>
            <input value="夜南香" />
            <div id="delegated-results" role="listbox">
              <div role="option" data-store-id="poi-1">
                <span data-store-name>夜南香银滩店</span>
                <span data-store-address>广西北海市银海区银滩路 1 号</span>
              </div>
            </div>
            <div id="delegated-load-row"><span>点击加载更多</span></div>
          </div>
        </section>
        <script>
          document.addEventListener('click', event => {
            if (!event.target.closest('#delegated-load-row')) return;
            const row = document.createElement('div');
            row.setAttribute('role', 'option');
            row.setAttribute('data-store-id', 'poi-2');
            row.innerHTML = '<span data-store-name>夜南香侨港店</span>'
              + '<span data-store-address>广西北海市银海区侨港路 2 号</span>';
            document.querySelector('#delegated-results').appendChild(row);
          });
        </script>
        """
        async with async_playwright() as playwright:
            ...
            try:
                ...
                first_page = (
                    douyin_commerce_service.normalize_commerce_location_candidates(
                        await douyin_commerce_service._store_option_descriptors(
                            page.locator("#delegated-results")
                        )
                    )
                )
    ...
    async def test_load_more_reappear_wait_propagates_its_deadline(self) -> None:
        """按钮重现等待到期必须上抛，不能伪装为平台已无更多。"""
        ...
        with patch.object(
            ...
            douyin_commerce_service,
            "_wait_publish_location_timeout",
            new_callable=AsyncMock,
            side_effect=douyin_commerce_service.DouyinCommerceError(
                "publish_location_load_more_limit"
            ),
            ...
            with self.assertRaisesRegex(
                ...
                await douyin_commerce_service._wait_for_reappearing_load_more_control(
                    object(),
                    deadline=1.0,
                )
    ...
    async def test_load_more_returns_zero_after_stable_platform_count_change(
        self,
    ) -> None:
        """平台数量已变化且候选稳定时，可确认本轮响应为零个有效新增。"""
        ...
        self.assertEqual(snapshot.await_count, 4)
...
class DouyinCommerceUiTests(unittest.TestCase):
    ...
    def test_navigation_has_douyin_commerce_entry(self) -> None:
        ...
        try:
            ...
            self.assertEqual(labels[5], "抖音图文矩阵")
            self.assertEqual(labels[6], "抖音带货")
...
class DouyinCommerceSessionContractTests(unittest.TestCase):
    def _location_cache_batch(
        self,
        *,
        location_preset: dict[str, object] | None = None,
    ) -> dict[str, object]:
    ...
    def test_publish_uses_frozen_session_account_id_after_registry_rebind(self) -> None:
        """发布期间台账重绑且出现同 basename 时，回写仍须使用启动会话的账号。"""

        batch = self._location_cache_batch()

        class FrozenIdentityManager(FakeCommerceSessionManager):
            def start_upload(self, payload: dict, **kwargs) -> dict:
                result = super().start_upload(payload, **kwargs)
                return {**result, "accountId": 31}

        manager = FrozenIdentityManager()
        executor = DouyinCommerceBatchExecutor(
            manager,
            task_store=MagicMock(),
        )
        rebound_accounts = [
            {
                "id": 88,
                "type": 3,
                "filePath": "/rebound/one/douyin-31.json",
            },
            {
                "id": 99,
                "type": 3,
                "filePath": "/rebound/two/douyin-31.json",
            },
        ]
        with patch.object(
            executor,
        ...
        self.assertEqual(result["status"], "published")
        self.assertEqual(record_result.call_args.args[0], "31")
    ...
    def test_exhausted_location_records_failure_but_network_failure_does_not(self) -> None:
        ...
        class LocationFailureManager(FakeCommerceSessionManager):
            def apply_saved_location(self, *_args, **_kwargs):
                raise douyin_commerce_session.DouyinCommerceSessionError(
                    "publish_location_not_found_after_all_pages"
                )
    ...
    def test_interaction_and_cleanup_location_errors_do_not_touch_cache(self) -> None:
        """纯交互或面板清理失败不得被 batch 误报为 POI 失效。"""
        ...
        for error_code in (
            ...
            with self.subTest(error_code=error_code):
                class LocationInteractionFailureManager(FakeCommerceSessionManager):
                    def apply_saved_location(self, *_args, **_kwargs):
                        raise RuntimeError(error_code)

                executor = DouyinCommerceBatchExecutor(
                    LocationInteractionFailureManager(),
                    task_store=MagicMock(),
                )
                with patch(
                    "app_core.douyin_commerce_batch_executor.record_location_publish_result"
    ...
    def test_session_location_search_forwards_optional_absolute_deadline(self) -> None:
        manager = douyin_commerce_session.DouyinCommerceSessionManager()
        page = MagicMock()
        page.is_closed.return_value = False
        manager._session = douyin_commerce_session._CommerceEditorSession(
        ...
        candidate = self._session_location_candidates(1)[0]
    ...
    def test_initial_setup_search_caps_first_hundred_unique_identities(
        ...
        """首屏一次返回 101 个身份时，会话只保存稳定顺序的前 100 个。"""

        manager = douyin_commerce_session.DouyinCommerceSessionManager()
        page = MagicMock()
        page.is_closed.return_value = False
        manager._session = douyin_commerce_session._CommerceEditorSession(
            session_id="session-initial-limit",
            upload_payload={},
            account_name="测试账号",
            browser=None,
            context=None,
        ...
        initial = self._session_location_candidates(101)
        with patch.object(
            manager,
            "_call",
            side_effect=lambda coroutine: asyncio.run(coroutine),
        ...
        self.assertEqual(len(result["candidates"]), 100)
        self.assertEqual(
            [row["poiId"] for row in result["candidates"]],
            [row["poiId"] for row in initial[:100]],
        )
        self.assertFalse(result["hasMore"])
        self.assertEqual(result["stopReason"], "candidate_identity_limit")
        self.assertEqual(
        ...
        self.assertEqual(
            manager._session.location_search_context.candidates,
            initial[:100],
        )
        self.assertNotIn(
            initial[100]["poiId"],
            {
                row["poiId"]
                for row in manager._session.commerce_location_candidates
            },
        )
    ...
    def test_real_batch_preset_preserves_observed_commission_type_through_session_apply(
        self,
    ) -> None:
        """UI 批次快照只有 observed 字段时，真实会话仍须按四字段佣型匹配。"""

        class OpenPage:
            def is_closed(self) -> bool:
                return False

        for observed_type in ("commission", "no_commission"):
            with self.subTest(observed_type=observed_type):
                ui_saved_preset = {
                    "poiId": f"visible-poi:{observed_type}",
                    "name": f"{observed_type} 地点",
                    "address": f"广西北海市测试路 {observed_type} 号",
                    "scope": "domestic",
                    "commissionFilter": observed_type,
                    "observedCommissionType": observed_type,
                }
                self.assertNotIn("commissionType", ui_saved_preset)
                batch = self._location_cache_batch(
                    location_preset=ui_saved_preset,
                )
                payload = douyin_commerce_batch_service.item_publish_payload(
                    batch,
                    batch["items"][0],
                )
                manager = douyin_commerce_session.DouyinCommerceSessionManager()
                manager._session = douyin_commerce_session._CommerceEditorSession(
                    session_id="session-real-batch-preset",
                    upload_payload={},
                    account_name="测试账号",
                    browser=None,
                    context=None,
                ...
                keyword = ui_saved_preset["address"]
                ...
                async def apply_atomic(
                    ...
                    *,
                    ...
                    self.assertEqual(commission_filter, observed_type)
                    return {
                        "location": dict(preset),
                ...
                with patch.object(
                    douyin_commerce_session.douyin_commerce_service,
                    "apply_saved_commerce_location_to_page",
                    new=AsyncMock(side_effect=apply_atomic),
                ) as apply_saved:
                    manager.apply_saved_location(
                        "session-real-batch-preset",
                        payload["locationPoi"],
                        payload["locationScope"],
                        [keyword],
                        payload["locationCommissionFilter"],
                    )

                actual_preset = apply_saved.await_args.args[1]
                self.assertEqual(
                    actual_preset["commissionType"],
                    observed_type,
                )
    ...
    def test_transient_missing_scope_panel_waits_then_retries_without_music(self) -> None:
        ...
        expected = [
            {
                "poiId": "poi-transient-scope",
                "name": "北海夜南香",
                "address": "广西北海市银海区示例路1号",
                "commissionType": "no_commission",
            }
        ]
...
class DouyinCommerceBatchUiTests(unittest.TestCase):
    ...
    def test_hundred_no_commission_rows_do_not_finish_province_plan(self) -> None:
        """省份的 100 上限只能计入地区、返佣和完整身份都通过的候选。"""

        self._install_province_fixture("广东joymark")
        rows = [
            self._province_candidate(
                f"no-commission-{index:03d}", commission="no_commission"
            )
            for index in range(100)
        ]
        self.page._batch_location_search_succeeded(
            "domestic",
            "广东joymark",
            self._province_page(rows, has_more=False),
            "commission",
    ...
    def test_progress_write_start_failure_keeps_current_city(self) -> None:
        ...
        self.page._accept_province_location_page(
            owner, self._province_page([], has_more=False),
            started_at=time.monotonic(), actions_used=1,
        )

        self.assertEqual(self.page._batch_location_state()["searchPlan"].current_index, before.current_index)
        self.assertTrue(self.page._batch_location_state()["hasMore"])

    def test_progress_merge_or_save_failure_keeps_current_city(self) -> None:
        owner = self._install_province_fixture("广东joymark")
        before = self.page._batch_location_state()["searchPlan"]
        for failing_call in (
            "merge_platform_locations_for_queries",
            ...
            with self.subTest(failing_call=failing_call), patch.object(
                douyin_location_cache, failing_call, side_effect=RuntimeError("write failed")
            ):
                self.page._accept_province_location_page(
                    owner, self._province_page([], has_more=False),
                    started_at=time.monotonic(), actions_used=1,
    ...
    def test_progress_merges_root_and_active_city_before_saved_plan_replay(self) -> None:
        owner = self._install_province_fixture("广东joymark")
        original = self.page._batch_location_state()["searchPlan"]
        city_plan = replace(original, current_index=1)
        state = self.page._batch_location_state()
        state.update({"searchPlan": city_plan, "activeKeyword": city_plan.current_keyword})
        self.page._batch_location_searches["__shared_location_search__"] = state
        with patch.object(douyin_location_cache, "merge_platform_locations_for_queries") as merge, patch.object(
            douyin_location_cache, "save_location_search_plan"
        ) as save:
            self.page._accept_province_location_page(
                owner,
                self._province_page([self._province_candidate("replay")], has_more=False),
                started_at=time.monotonic(),
                actions_used=3,
            )

        queries, rows = merge.call_args.args
        self.assertEqual([query.keyword for query in queries], ["广东joymark", city_plan.current_keyword])
        self.assertEqual(rows[0]["poiId"], "replay")
        saved_plan = save.call_args.args[1]
        self.assertEqual(saved_plan.current_index, 2)
        self.assertEqual(self.page._batch_location_state()["searchPlan"], saved_plan)

    def test_deadline_after_first_action_restores_retry_without_next_action(self) -> None:
        owner = self._install_province_fixture("广东joymark")
        calls = 0

        def unexpected_next(*_args, **_kwargs):
            nonlocal calls
            calls += 1
            return False

        with patch.object(self.page, "_run_current_location_action", side_effect=unexpected_next):
            self.page._accept_province_location_page(
                owner,
                self._province_page([], has_more=False),
                started_at=time.monotonic() - 31,
                actions_used=1,
            )
    ...
    def test_cache_search_silently_pages_to_exhaustion_and_second_miss_invalidates(
        ...
        database_patch = patch.object(
            database,
            "DB_PATH",
            Path(temporary_directory.name) / "revalidation.sqlite3",
        )
        database_patch.start()
        self.addCleanup(database_patch.stop)
        account_id = "812"
        ...
        first_seen = {
            "poiId": "poi-platform-a",
            "name": "平台地点 A",
            "address": "广西北海市平台路 1 号",
            "commissionType": "commission",
        }
        second_seen = {
            "poiId": "poi-platform-b",
            "name": "平台地点 B",
            "address": "广西北海市平台路 2 号",
            "commissionType": "commission",
        }
        ...
        self.page.account_combo.clear()
        self.page.account_combo.addItem("请选择账号", None)
        self.page.account_combo.addItem("受控测试账号", account)
        self.page.account_combo.setCurrentIndex(1)
        self.page._setup_generation_id = ""
        self.page._session_id = "session-revalidation"
        runner = self._ControlledLifecycleRunner()
        self.page.runner = runner
        self.page.batch_location_commission_combo.setCurrentIndex(
            self.page.batch_location_commission_combo.findData("commission")
        ...
        def cache_row() -> tuple[str, int]:
            ...
            return str(row["status"]), int(row["revalidationFailures"])

        def next_page(*_args, **_kwargs):
            if load_more.call_count % 2 == 1:
                return {
                    "platformResultCount": 2,
                    "candidates": [first_seen, second_seen],
                    "newCandidateCount": 1,
                    "hasMore": True,
                    "stopReason": "loaded",
                }
        ...
        with patch(
            ...
            for expected_failures in (1, 2):
                ...
                self.assertEqual(cache_row(), ("needs_revalidation", expected_failures - 1))
                runner.finish(self.page._IMMEDIATE_WRITE_KEY)

                runner.execute(self.page._IMMEDIATE_WRITE_KEY)
                self.assertEqual(cache_row(), ("needs_revalidation", expected_failures - 1))
                self.assertEqual(reconcile.call_count, expected_failures - 1)
                runner.finish(self.page._IMMEDIATE_WRITE_KEY)
                ...
                expected_status = (
                    "invalid" if expected_failures == 2 else "needs_revalidation"
                )
                self.assertEqual(cache_row(), (expected_status, expected_failures))

        self.assertEqual(platform_search.call_count, 2)
        self.assertEqual(load_more.call_count, 4)
        self.assertEqual(reconcile.call_count, 2)
        for reconciliation_call in reconcile.call_args_list:
            self.assertEqual(
                reconciliation_call.args[1],
                [first_seen, second_seen],
            )
            self.assertIs(
                reconciliation_call.kwargs["confirmed_exhausted"],
                True,
            )

    def test_cache_search_interaction_failure_never_marks_missing(
        self,
    ) -> None:
        """静默校对交互失败只能保留待校对，不能把部分集合当作穷尽。"""
        ...
        self.page.account_combo.setCurrentIndex(1)
        self.page._setup_generation_id = ""
        self.page._session_id = "session-revalidation-failure"
        self.page.runner = self._ControlledLifecycleRunner()
        self.page.batch_location_commission_combo.setCurrentIndex(
            self.page.batch_location_commission_combo.findData("commission")
        )
        ...
        cache_page = self._cache_page(cached, requires_revalidation=True)
        first_seen = {
            "poiId": "poi-platform-partial",
            "name": "平台部分地点",
            "address": "广西北海市平台路 9 号",
            "commissionType": "commission",
        }
        ...
        with patch.object(
            ...
            self.assertIn(
                self.page._IMMEDIATE_WRITE_KEY,
    ...
    def test_initial_platform_search_caps_ui_and_cache_merge_before_autofill(
        self,
    ) -> None:
        """真实首屏回调不得让第 101 个身份进入状态、下拉或缓存合并。"""

        self._activate_cached_location_search(account_id=510)
        cache_query = self.page._batch_location_cache_query(
            "domestic", "北海", "commission"
        )
        cached = self._cached_location_candidates(10)
        ...
        path = "/tmp/initial-platform-limit.mp4"
        self.page.video_combo.clear()
        self.page.video_combo.addItem("请选择视频", None)
        self.page.video_combo.addItem(
            "initial-platform-limit.mp4",
        ...
        initial = [
            {
                **row,
                "poiId": f"poi-platform-{index:03d}",
                "name": f"平台首屏地点 {index:03d}",
                "address": f"北海市平台首屏路 {index:03d} 号",
            }
            for index, row in enumerate(self._cached_location_candidates(101))
        ]
        excluded_poi = initial[100]["poiId"]
        original_auto_fill = self.page._auto_fill_batch_location_candidates
        ...
        def assert_bounded_before_auto_fill(scope, candidates, **kwargs):
            ...
            self.assertNotIn(
                excluded_poi,
                {row["poiId"] for row in state["candidates"]},
            )
            return original_auto_fill(scope, candidates, **kwargs)

        with patch.object(
            self.page,
            "_auto_fill_batch_location_candidates",
            side_effect=assert_bounded_before_auto_fill,
        ), patch.object(
            self.page,
            "_start_batch_location_cache_merge",
            return_value=True,
        ...
        option_pois = {
            str((dropdown.itemData(index) or {}).get("poiId") or "")
    ...
    def test_real_ui_selection_records_frozen_cache_identity_and_survives_eviction(
        self,
    ) -> None:
        """真实选择入口须异步记录原账号身份，并让该行优先留在 100 条内。"""
        ...
        database_patch = patch.object(
            database,
            "DB_PATH",
            Path(temporary_directory.name) / "selection.sqlite3",
        )
        database_patch.start()
        self.addCleanup(database_patch.stop)
        account_id = "914"
        ...
        self.assertEqual(len(selection_keys), 1, runner.active)
        self.page.account_combo.setCurrentIndex(2)
        self.page._batch_location_search_token += 1
        self.page.batch_location_keyword.setText("迟到关键词")
        selected["poiId"] = "mutated-after-dispatch"
        selected["name"] = "迟到突变"
        newcomer = {
            **initial[0],
            "poiId": "poi-cache-newcomer",
            "name": "新进入地点",
            "address": "北海市新进入路 1 号",
        }
        ...
        with database.connect() as conn:
            links_before_selection = conn.execute(
                """
                ...
                (
                    ...
                    original_identity["poiId"],
                ),
            ).fetchone()["count"]
        self.assertEqual(links_before_selection, 0)

        runner.execute(selection_keys[0])
        runner.finish(selection_keys[0])

        saved = self.page._batch_locations[path]
        self.assertEqual(
            {key: saved[key] for key in ("poiId", "name", "address")},
            {
                key: original_identity[key]
                for key in ("poiId", "name", "address")
            },
        )
        retained = {
        ...
        self.assertIn(original_identity["poiId"], retained)
        self.assertIn(newcomer["poiId"], retained)
        with database.connect() as conn:
            selected_row = conn.execute(
                "SELECT lastSelectedAt, verifiedAt FROM douyin_location_cache "
                "WHERE accountId = ? AND poiId = ?",
                (account_id, original_identity["poiId"]),
            ).fetchone()
    ...
    def test_real_ui_selection_cache_failure_keeps_selection_and_controlled_feedback(
        ...
        """选择生命周期写入失败不得撤销已选地点或向 UI 暴露原始异常。"""
        ...
        self.page.account_combo.addItem(
            "测试账号",
            {
                ...
                "type": 3,
                "status": 1,
                "filePath": "douyin-916.json",
            },
        )
        self.page.account_combo.setCurrentIndex(0)
        candidate = {
            "poiId": "poi-selection-failure",
            "name": "选择失败保护地点",
            "address": "北海市失败保护路 1 号",
            "commissionType": "commission",
        }
        self.page._batch_location_searches["__shared_location_search__"] = {
            "accountId": "916",
            ...
            "keyword": "北海",
            "commissionFilter": "commission",
            "rawCandidates": [dict(candidate)],
            "candidates": [dict(candidate)],
            "platformCandidates": [dict(candidate)],
            "hasMore": False,
        }
        runner = self._ControlledLifecycleRunner()
        self.page.runner = runner
        path = "/tmp/selection-cache-failure.mp4"
        with patch(
            "ui.douyin_commerce_page.save_location_preset",
            return_value={
                "id": "preset-selection-failure",
            ...
            douyin_location_cache,
            "record_location_selection",
            side_effect=RuntimeError("/private/cache/raw-selection-error"),
        ):
            self.page._select_batch_location_candidate(
                path,
                "domestic",
        ...
        self.assertEqual(
            self.page._batch_locations[path]["poiId"],
            candidate["poiId"],
        )
        self.assertEqual(
            self.page._batch_location_feedback,
