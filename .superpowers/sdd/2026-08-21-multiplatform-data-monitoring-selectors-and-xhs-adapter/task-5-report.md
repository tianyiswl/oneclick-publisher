...
- 追加一条真实离线全链测试：仅替换平台 1 的采集器工厂，不替换同步落库或页面查询。它使用临时 SQLite 实际执行 `sync_account_data`，再由真实 Qt 页面从同一数据库读取账号指标和已确认的 0 作品；随后用 `browser_cleanup_incomplete` 验证最新运行变为失败、历史成功指标不被污染。
...
- 本任务只完成本地注入采集器、SQLite/离屏验证；没有打开真实小红书会话、没有访问平台或真实账号，因此不构成真实平台同步或数据回读证据。
