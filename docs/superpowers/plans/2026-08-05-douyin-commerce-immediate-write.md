...
在 _build_music_stage() 中用单个 QComboBox 替换候选列表和“确认使用”按钮。为避免真实抖音的音乐选择弹层在未操作时阻塞定位、声明和定时，用户首次打开下拉或点击刷新时才读取收藏音乐；读取完成后直接展开本地下拉，但不自动选择。下拉变更忽略空值和信号阻塞期，其余候选直接调用 _start_music_write()；成功后显示回读值，失败后恢复已确认值。
